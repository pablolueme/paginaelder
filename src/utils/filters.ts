...
const playerBuildSearchBlob = (build: PlayerBuildEntry): string => {
  ...
  return normalize(
    [
      ...
      (build.seals ?? []).map((item) => `${item.nameEs} ${item.nameEn}`).join(" "),
      build.sealNote ?? "",
      (build.incantations ?? []).map((item) => `${item.nameEs} ${item.nameEn}`).join(" "),
      build.incantationNote ?? "",
      (build.gameplayLoop ?? []).join(" "),
      (build.weaponSetupNotes ?? []).join(" "),
      build.armorSet ? `${build.armorSet.nameEs} ${build.armorSet.nameEn}` : "",
      build.armorNote ?? "",
